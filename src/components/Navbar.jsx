          ...
          <div className="hidden md:flex gap-8 text-sm font-medium ">
            {['home', 'about', 'projects', 'contact'].map((item) => (
              <button
                ...
                className={`capitalize transition-colors hover:text-blue-400 ${activeSection === item ? 'text-blue-400' : 'text-slate-400'}`}
              >
                {item}
              </button>
          ...
          <div className="md:hidden bg-slate-900 p-4 border-b border-slate-800 animate-in slide-in-from-top duration-300">
            {['home', 'about', 'projects', 'contact'].map((item) => (
              <button
                key={item}
                onClick={() => scrollTo(item)}
                className="block w-full text-left py-3 capitalize text-slate-300 hover:text-white"
              >
                {item}
              </button>
            ))}
            <button
              ...
              className="mt-4 w-full px-4 py-3 bg-blue-600 text-white rounded-lg text-sm font-bold text-center"
            >
              RESUME PDF
            </button>
          </div>
